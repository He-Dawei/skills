...
## Environment Setup (IMPORTANT — read before executing any commands)

### Current Setup: Node.js CLI (no conda needed)

`mineru-open-api` is installed globally via npm:
```
npm install -g mineru-open-api
```

**No conda environment required.** The command is available system-wide. Execute `mineru-open-api` directly.
...
```
Expected: `mineru-open-api version v0.5.9` or newer.

### Alternative: Python mineru[pipeline] (conda required)

If the user prefers **local offline processing** (no API calls to mineru.net), use the Python package instead. This REQUIRES a conda environment:
...
# Every subsequent use — MUST remind the user:
conda activate mineru
mineru -p "FILE" -o "OUTPUT_DIR/" -b pipeline
```

**CRITICAL**: Before running any `mineru` command, check if `conda activate mineru` is needed. If the user has the Python pipeline installed, ALWAYS remind them to activate the conda environment first. If the conda env is not yet created, guide them through the one-time setup above.

### How to Check Which Setup is Active

```bash
# Check if Node.js CLI is available (runs anywhere)
mineru-open-api --version

# Check if Python pipeline is available (needs conda)
conda activate mineru && mineru --version
```

> **Default**: Use `mineru-open-api` (Node.js, no env hassle). Fall back to Python `mineru` only when the user explicitly wants local processing or has no network access.
...
- **`mineru` command not found:** Python pipeline not in PATH. Remind user: `conda activate mineru` first, then retry.
- **HuggingFace model download slow/blocked:** Switch to ModelScope mirror: `$env:MINERU_MODEL_SOURCE="modelscope"` (Windows) or `export MINERU_MODEL_SOURCE=modelscope` (macOS/Linux) before running `mineru`.
